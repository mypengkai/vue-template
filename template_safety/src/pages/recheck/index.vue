<template>
  <div class="recheck">
    <van-nav-bar fixed title="安全复核" left-text="返回" left-arrow @click-left="$router.push('/work')" />
    <div class="conent">
      <!-- 搜索 -->
      ...
      >
        <ul v-for="(item,index) in recheckList" :key="index" @click="routerDetail(item)">
  ...
  data() {
    return {
      ...
      formData: {
        ...
        spEndDate: "", // 结束时间
        isOverdue: "", // 逾期 状态
        rectificationState: "1" // 整改状态（0待整改1待复检）
      }
    };
  },
  created() {
      ...
      reform.getReformList(this.formData).then(res => {
        if (res.data.success == 0) {
          ...
          if (this.formData.offset == 0) {
            this.recheckList = res.data.rows;
          } else {
            this.recheckList = [...this.recheckList, ...res.data.rows];
          }
          this.noData = this.recheckList.length == res.data.total;
        }
      });
    },
    onLoad() {
